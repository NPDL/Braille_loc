...
cedrus.gettime = @cedrusgettime;
% cedrus.event = {}; %changed by SK
cedrus.event = [];
...
try
    ...
    try
        cedrus.port = serial("COM5", 'BaudRate', 115200, 'DataBits', 8, ...
            'StopBits', 1, 'FlowControl', 'none', 'Parity', 'none', ...
            'BytesAvailableFcnMode', 'byte', 'BytesAvailableFcnCount', 6, ...
            'BytesAvailableFcn', @cedrusread);
        fopen(cedrus.port);
    catch
        cedrus.port = serial("COM6", 'BaudRate', 115200, 'DataBits', 8, ...
            'StopBits', 1, 'FlowControl', 'none', 'Parity', 'none', ...
            'BytesAvailableFcnMode', 'byte', 'BytesAvailableFcnCount', 6, ...
            'BytesAvailableFcn', @cedrusread);
        fopen(cedrus.port);
...
%Get time from cedrus --written by shipra--doesn't work
function cedrusgettime
global cedrusTime;
if exist('timercounter','var')==0;
    timercounter=0;
end
timercounter=timercounter+1;
r = uint32(fread(cedrus.port,6));
cedrusTime(timercounter) = ((r(6) * 256 + r(5)) * 256 + r(4)) * 256 + r(3);
...
if press || cedrus.releases
  ...
  time=double(time);
  press=double(press);
  ptbtime=double(ptbtime);
  
  cedrus.count = cedrus.count + 1;
  %cedrus.event{cedrus.count} = [button time press ptbtime]; %changed by SK
  cedrus.event(cedrus.count,:) = [button time press ptbtime];
...
function [button time press ptbtime] = cedrusgetpress
global cedrus;
if cedrus.count > 0
%   ptbtime=GetSecs;
%   button = cedrus.event{1}(1); %changed by SK
%   time   = cedrus.event{1}(2);
%   press  = cedrus.event{1}(3);
%   ptbtime = cedrus.event{1}(4);
  ...
  button = 0; time = 0; press = 0; ptbtime=0;
end

% wait for a button event
function [button time press ptbtime] = cedruswaitpress (varargin)
global cedrus;
button=0; time=0; press=0; ptbtime=0;
% any events already stored?
% [button time press ptbtime] = cedrusgetpress; %SK commented this out on 10/10/16 because
% we dont want any previous responses to count
...
if button == 0
  ...
  start (cedrus.timer);
  wait (cedrus.timer);
  [button time press ptbtime] = cedrusgetpress;
end
